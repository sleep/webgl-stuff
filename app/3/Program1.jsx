...
vec3 shadeSphere(vec3 point, vec4 sphere, vec3 material) {
  ...
  float shinyness = 2.;
  vec3 reflected = 2. * (dot(-1. * Ldir, normal)) * normal - (-1. * Ldir);
  vec3 eye = -1. * normalize(point);


  color = a_rgb;
  color += Lrgb * d_rgb* max(0., dot(normal, -1. * Ldir));
  color += Lrgb * s_rgb* pow(max(0., dot(eye, reflected)), shinyness);
