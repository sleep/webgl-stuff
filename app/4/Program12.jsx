...
Surface getSurface(vec3 V, vec3 W, World world) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (t < surf.t) {
      ...
      surf.reflectance = inside ? 0.0 : world.sphere[i].reflectance;
...
vec3 rayTrace(vec3 V_initial, vec3 W_initial, World world) {
  ...
  for (int i = 0; i < 9; i++) {
    ...
    for (int i = 0; i < 2; i ++) {
      ...
      _color += shadeWithShadows(surf, light, world) *  min(1., 5.0/t_total);
    }
    ...
    if (canReflect) {
      if (canTransmit) {
        // bool flip =  rand(sin(time) * W.xy) > 0.5;
        // bool flip =  sin(time) > 0.;
        bool flip =  2.0 *rand(sin(time) * W.xy) - 1. > 2.0 * sin(time/10.);
        action = flip
          ? 1
          : 2;
        // action = 2;
...
void main(void) {
  ...
  for (int i = 0; i< 3; i++) {
    ...
    world.sphere[i].opacity = 0.01;
    world.sphere[i].eta = 1.1;
  }
